...
import telebot
import wb_api
from wb_applications.models import Account
...
def cancel_number(number_id, activate_type):
    if activate_type == 'rent':
        url = 'https://api.sms-activate.org/stubs/handler_api.php'
        response = requests.get(
            url,
            params={
...
def get_sms_code(number_id, activate_type):
    if activate_type == 'rent':
        url = 'https://api.sms-activate.org/stubs/handler_api.php'
        for _ in range(6):
            time.sleep(10)
...
def register_accounts(count, activate_type):
    for i in range(int(count)):
        for _ in range(3):
            number_id, phone_number = get_number(activate_type)
            if number_id is not None:
                print(f'Регистрирую аккаунт {i} с номером {phone_number}...')
                try:
                    result = register_account(phone_number, number_id, activate_type)
                except Exception as error:
                    result = [False, traceback.format_exc()]
                if result[0]:
                    Account.objects.create(profile_name=phone_number, payment_qr=False, active=True)
                    print(f'Успешно {i} | {result[1]}. {phone_number}')
                    break
                else:
                    cancel_number(number_id, activate_type)
                    print(f'Ошибка {i} | {result[1]}. {phone_number}')


def register_account(phone_number, number_id, activate_type):
    ...
    sms_code = get_sms_code(number_id, activate_type)
    ...
    time.sleep(2)
    pickle.dump(browser.get_cookies(), open(f'wb_applications/static/cookies_{phone_number}.pkl', "wb"))
    browser.quit()
